...
> **Port note (X570D4I-2T).** This is upstream's 2019 design document, kept for
> the architecture overview. Two parts no longer describe the shipped code:
>
> - **Legacy `Mount` signature.** The table below documents
>   `STRING / BOOLEAN / VARIANT<UNIX_FD,INT>` with `-1` passed as `INT` when
>   there is no secret. No released bmcweb ever sent that `INT` arm — before
>   commit `4bbf2a13c2` it sent a single-alternative `variant<unix_fd>`, and
>   since that commit it sends a bare `unix_fd` and always creates the pipe.
>   Patch `0003` moves the daemon to the current `sbh` form to match.
> - **Configuration.** This board uses `Slot_0`..`Slot_3` rather than `ISO0`,
>   sockets under `/run/virtual-media/`, and an added per-slot `Cdrom` key
>   (patch `0002`) that gives the Redfish `Cd` boot override a CD-class device
>   to resolve against. `BlockSize` is parsed but never used by the daemon.
