...
# ---------------------------------------------------------------------------
# This package ships ONE ipmid provider library and nothing else (2026-08-03)
# ---------------------------------------------------------------------------
# It previously also shipped asrock-bios-nvram plus a flash-sync script and a
# power-state-triggered flash-monitor service, which pulled the BIOS SPI mux,
# bound spi1.0 and read/wrote /dev/mtd6. All of that is gone -- see the "NO
# DIRECT FLASH ACCESS" note in meson.build. Consequently there is no systemd
# unit here any more, hence no `systemd` inherit and no SYSTEMD_SERVICE.
#
# BIOS attribute values now arrive from the BIOS over IPMI (NetFn 0x30) rather
# than being scraped out of the SPI flash behind the firmware's back.
