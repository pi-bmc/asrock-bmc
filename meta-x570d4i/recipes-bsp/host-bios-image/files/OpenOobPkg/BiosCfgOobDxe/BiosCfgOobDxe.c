...
/* Driver-specific telemetry bits (high half; the low half is shared — see
   OobTelemetryLib.h). Recorded to an NV variable because neither POST codes nor
   the BMC's IPMI journal are trustworthy on this board. */
#define BIOSCFG_TLM_CAP_SENT       0x00010000u
#define BIOSCFG_TLM_APPLIED        0x00020000u
#define BIOSCFG_TLM_RESET_ISSUED   0x00040000u
#define BIOSCFG_TLM_SCHEMA_SENT    0x00080000u
#define BIOSCFG_TLM_SNAPSHOT_SENT  0x00100000u
  ...
  )
{
  ...
  Status = IpmiKcsRawTransaction (Req, i, Raw, &RawLen);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  if (RawLen < 3) {
  ...
  )
{
  ...
  {
    OobPostCode (0x6C);
    OobTelemetryDetail (0, EFI_DEVICE_ERROR, Cc);
    OobTelemetryFlag (OOB_TLM_KCS_FAIL);
    return;
  }
  OobTelemetryDetail (0, EFI_SUCCESS, Cc);
  OobTelemetryFlag (OOB_TLM_KCS_OK | BIOSCFG_TLM_CAP_SENT);
  ...
  if (ApplyPending (&Applied, &Generation) && (Applied != 0)) {
    OobPostCode (0x67);
    OobTelemetryDetail (Applied, EFI_SUCCESS, Cc);
    OobTelemetryFlag (BIOSCFG_TLM_APPLIED);
    ...
    if (!mOobResetDone && (Generation != 0) && (Generation != LastGen)) {
      ...
      OobPostCode (0x68);
      /* Flag before resetting, not after: OobTelemetryFlag persists
         synchronously, and nothing after ResetSystem executes. Without this the
         reset would be invisible in the record and look like a silent hang. */
      OobTelemetryFlag (BIOSCFG_TLM_RESET_ISSUED);
  ...
  OobTelemetryFlag (BIOSCFG_TLM_SCHEMA_SENT | (Skipped ? OOB_TLM_XFER_SKIP : 0));
  ...
  OobPostCode (0x61);
  OobTelemetryDetail (SnapLen, EFI_SUCCESS, Cc);
  ...
  FreePool (Snap);
  OobPostCode (Skipped ? 0x63 : 0x62);
  OobTelemetryFlag (
    BIOSCFG_TLM_SNAPSHOT_SENT | (Skipped ? OOB_TLM_XFER_SKIP : OOB_TLM_XFER_OK)
    );
...
/* ── driver lifecycle ─────────────────────────────────────────────────────────
   This module used to be a passenger: SmbiosBmcPushDxe owned the entry point
   and called BiosCfgOobInit/Run/IsDone from its own EndOfDxe / timer /
   ReadyToBoot hooks, and this file borrowed its KcsTxn/Post. Both directions of
   that coupling are gone — the transport now comes from IpmiKcsLib and the
   driver arms its own events below.

   The retry timer exists because the BMC is frequently still booting when DXE
   starts, so the first attempt often fails on KCS not being ready. 20 s period,
   600 ticks, and it stands down as soon as the exchange completes. */

STATIC UINTN      mOobTicks = 0;
STATIC EFI_EVENT  mOobTimer = NULL;
...
{
  OobTelemetryFlag (OOB_TLM_READYTOBOOT);
  BiosCfgOobRun ();
}
...
{
  OobTelemetryFlag (OOB_TLM_ENDOFDXE);
  BiosCfgOobRun ();
}

/* Late-DXE protocol installs are the only retry mechanism that fires on this
   board: the periodic timer below reports TIMER_ARMED and then Ticks == 0
   forever, and EndOfDxe/ReadyToBoot never arrive either. A protocol notify is
   signalled synchronously inside InstallProtocolInterface, so it needs neither
   the timer tick nor BDS. See host-bios-driver-injection-results.md. */
...
STATIC
VOID
ArmOobProtocolNotify (
  IN EFI_GUID  *Protocol
  )
{
  VOID  *Registration;

  EfiCreateProtocolNotifyEvent (Protocol, TPL_CALLBACK,
                                OnOobLateProtocol, NULL, &Registration);
}
...
/**
  Entry point. Arms the same three hooks the combined driver used to share.

  Note the ordering constraint this module has and SmbiosBmcPushDxe does not:
  ApplyPending may issue a cold reset, and Setup has already consumed its
  variables by the time any of these hooks fire, so a staged Redfish write only
  takes effect after that reset. BiosCfgOobRun bounds it to one reset per
  BMC-side generation change, and never more than one per boot.
**/
EFI_STATUS
EFIAPI
BiosCfgOobEntry (
  ...
  )
{
  ...
  OobTelemetryInit (L"OobBiosCfg");
  OobTelemetryFlag (OOB_TLM_ENTRY);

  /* One immediate attempt, then the retries that actually fire here. */
  BiosCfgOobRun ();

  ArmOobProtocolNotify (&mPciEnumCompleteGuid);
  ArmOobProtocolNotify (&mDxeSmmReadyToLockGuid);
  ...
  {
    if (!EFI_ERROR (gBS->SetTimer (mOobTimer, TimerPeriodic, 20000000ULL))) {
